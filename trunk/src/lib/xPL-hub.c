...
/* Hub status */
static Eina_Bool hubRunning = EINA_FALSE;
...
/* Return EINA_TRUE if the received heartbeat message is on our local host */
static Eina_Bool isLocalClient(xPL_MessagePtr theMessage) {
  int ipIndex ;
  ...
  for (ipIndex = 0; ipIndex < localIPAddrCount; ipIndex++) {
    if (!strcmp(clientIPAddr, localIPAddrs[ipIndex])) return EINA_TRUE;
  }

  return EINA_FALSE;
}

/* Return EINA_TRUE if this is a heartbeat message */
static Eina_Bool isHeartbeatMessage(xPL_MessagePtr theMessage) {
  ...
  return ((strcasecmp(schemaClass, "hbeat") == 0)
	  || (strcasecmp(schemaClass, "config") == 0));
}

/* Return EINA_TRUE if this is a signoff  message */
static Eina_Bool isSignoffMessage(xPL_MessagePtr theMessage) {
  ...
  return (strcasecmp(schemaType, "end") == 0);
}
...
/* Given the passed client, remove it from our list and return EINA_TRUE.  If */
/* client is not found, return EINA_FALSE                                     */
static Eina_Bool releaseClient(hubClientPtr theClient) {
  int clientIndex;
...
static void checkForClientUpdates(xPL_MessagePtr theMessage) {
  ...
  Eina_Bool clientSigningOff = EINA_FALSE;
  hubClientPtr theClient = NULL;
...
/* well, return EINA_TRUE.  If there is an error, return EINA_FALSE               */
static Eina_Bool rebroadcastMessage(hubClientPtr theClient, String theData, int dataLen) {
  int bytesSent;
...
/* default).                                              */
Eina_Bool xPL_startHub() {
  /* If the hub is already running, we're done */
  if (hubRunning) return EINA_TRUE;
...
void xPL_stopHub() {
  ...
  /* Clear running flag */
  hubRunning = EINA_FALSE;
}

/* Return if hub is running or not */
Eina_Bool xPL_isHubRunning() {
  return hubRunning;
}
