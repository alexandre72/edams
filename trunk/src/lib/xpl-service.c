...
Eina_Bool xPL_sendHeartbeat(xPL_ServicePtr theService) {
  ...
  /* Send the message */
  if (!xPL_sendMessage(theHeartbeat)) return EINA_FALSE;
...
Eina_Bool xPL_sendGoodbyeHeartbeat(xPL_ServicePtr theService) {
  xPL_MessagePtr theHeartbeat;
...
void xPL_setServiceDeviceID(xPL_ServicePtr theService, String newDeviceID) {
  /* Skip if name is already this name */
  if ((theService->serviceDeviceID != NULL) && !strcasecmp(theService->serviceDeviceID, newDeviceID)) return;
...
void xPL_setServiceInstanceID(xPL_ServicePtr theService, String newInstanceID) {
  ...
  if ((theService->serviceInstanceID != NULL) && !strcasecmp(theService->serviceInstanceID, newInstanceID)) return;
...
void xPL_setReportOwnMessages(xPL_ServicePtr theService, Eina_Bool reportOwnMessages) {
  if (theService->reportOwnMessages == reportOwnMessages) return;
  theService->reportOwnMessages = reportOwnMessages;
}

Eina_Bool xPL_isReportOwnMessages(xPL_ServicePtr theService) {
...
void xPL_setRespondingToBroadcasts(xPL_ServicePtr theService, Eina_Bool respondToBroadcasts) {
  if (!theService->ignoreBroadcasts != respondToBroadcasts) return;
  theService->ignoreBroadcasts = !respondToBroadcasts;
}

Eina_Bool xPL_isRespondingToBroadcasts(xPL_ServicePtr theService) {
...
/* Return EINA_TRUE if the passed message matches the passed filter */
static Eina_Bool doesFilterMatch(xPL_ServiceFilterPtr theFilter, xPL_MessagePtr theMessage) {
  /* If there is a message type filter, skip out if it does not match the message */
  if ((theFilter->matchOnMessageType != xPL_MESSAGE_ANY) && (theFilter->matchOnMessageType != theMessage->messageType)) return EINA_FALSE;

  /* If there is a vendor filter and it doesn't match the message, skip the message */
  if ((theFilter->matchOnVendor != NULL) && (strcasecmp(theFilter->matchOnVendor, theMessage->sourceVendor) != 0)) return EINA_FALSE;

  /* If there is a device ID filter and it doesn't match the message, skip the message */
  if ((theFilter->matchOnDeviceID != NULL) && (strcasecmp(theFilter->matchOnDeviceID, theMessage->sourceDeviceID) != 0)) return EINA_FALSE;

  /* If there is a instance ID filter and it doesn't match the message, skip the message */
  if ((theFilter->matchOnInstanceID != NULL) && (strcasecmp(theFilter->matchOnInstanceID, theMessage->sourceInstanceID) != 0)) return EINA_FALSE;

  /* If there is a schema class filter and it doesn't match the message, skip the message */
  if ((theFilter->matchOnSchemaClass != NULL) && (strcasecmp(theFilter->matchOnSchemaClass, theMessage->schemaClass) != 0)) return EINA_FALSE;

  /* If there is a schema type filter and it doesn't match the message, skip the message */
  if ((theFilter->matchOnSchemaType != NULL) && (strcasecmp(theFilter->matchOnSchemaType, theMessage->schemaType) != 0)) return EINA_FALSE;

  /* Matches all selectors of this filter (if any) */
  return EINA_TRUE;
}

/* Return EINA_TRUE if we have any filters */
Eina_Bool xPL_isServiceFiltered(xPL_ServicePtr theService) {
...
/* Return EINA_TRUE if we have one or more groups */
Eina_Bool xPL_doesServiceHaveGroups(xPL_ServicePtr theService) {
...
Eina_Bool xPL_sendServiceMessage(xPL_ServicePtr theService, xPL_MessagePtr theMessage) {
  if ((theService == NULL) || (theMessage == NULL)) return EINA_FALSE;
...
void xPL_disableAllServices() {
  ...
  for (serviceIndex = 0; serviceIndex < serviceCount; serviceIndex++) {
    xPL_setServiceEnabled(serviceList[serviceIndex], EINA_FALSE);
  }
}
...
static void handleMessage(xPL_ServicePtr theService, xPL_MessagePtr theMessage) {
  ...
  if (theMessage->isBroadcastMessage) {
    ...
    if (theMessage->isGroupMessage) {
      ...
      for (groupIndex = 0; groupIndex < theService->groupCount; groupIndex++) {
	if (!strcasecmp(theMessage->groupName, theService->groupList[groupIndex])) {
      ...
      if ((strcasecmp(theMessage->targetVendor, theService->serviceVendor) != 0)
	  || (strcasecmp(theMessage->targetDeviceID, theService->serviceDeviceID) != 0)
	  || (strcasecmp(theMessage->targetInstanceID, theService->serviceInstanceID) != 0)) return;
    }
  }
