...
/* Status/state */
static Eina_Bool xPL_IODeviceInstalled = EINA_FALSE;
...
Eina_Bool xPL_addIODevice(xPL_IOHandler theIOHandler, int userValue, int theFD, Eina_Bool watchRead, Eina_Bool watchWrite, Eina_Bool watchError) {
  /* Make sure they are going to really do something */
  if (!watchRead && !watchWrite && !watchError) return EINA_FALSE;
...
/* fd exists, it's removed and EINA_TRUE is returned.  If    */
/* the fd doesn't exist, EINA_FALSE is returned.             */
Eina_Bool xPL_removeIODevice(int theFD) {
  int infoIndex;
  ...
  /* Never found it */
  debug(MSG_XPL,_("Can't remove managed IO device, fd=%d not found"), theFD);
  return EINA_FALSE;
}
...
Eina_Bool xPL_removeTimeoutHandler(xPL_TimeoutHandler timeoutHandler) {
  int timeoutIndex;
...
static Eina_Bool markNonblocking(int thefd) {
  int theValue;
...
/* we make it bigger, return EINA_TRUE.  Otherwise, if no change, EINA_FALSE */
static Eina_Bool maximizeReceiveBufferSize(int thefd) {
  ...
  if (getsockopt(thefd, SOL_SOCKET, SO_RCVBUF, &startRcvBuffSize, &buffLen) != 0)
    debug(MSG_XPL,_("Can't read receive socket buffer size - %s (%d)"), strerror(errno), errno);
  ...
  for (idealRcvBuffSize = 1024000; idealRcvBuffSize > startRcvBuffSize; ) {
    ...
    if (setsockopt(thefd, SOL_SOCKET, SO_RCVBUF, &idealRcvBuffSize, sizeof(int)) != 0) {
      debug(MSG_XPL, _("Can't to set receive buffer to %d bytes - retrying"), idealRcvBuffSize);
  ...
  debug(MSG_XPL,_("Can't increase receive buffer size - dang!"));
  return EINA_FALSE;
}

/* Attempt to make a standalone connection */
static Eina_Bool attemptStandaloneConnection() {
  int sockfd;
  int flag = 1;
  struct protoent *ppe;
  ...
  {
    debug(MSG_XPL, _("Can't to lookup UDP protocol info"));
    return EINA_FALSE;
  }
  ...
  {
    debug(MSG_XPL, _("Can't create listener socket %s (%d)"), strerror(errno), errno);
    return EINA_FALSE;
  }
  ...
  {
    debug(MSG_XPL, _("Can't set SO_REUSEADDR on socket %s (%d)"), strerror(errno), errno);
    close(sockfd);
    return EINA_FALSE;
  }
  ...
  if (setsockopt(sockfd, SOL_SOCKET, SO_BROADCAST, &flag, sizeof(flag)) < 0)
  {
    debug(MSG_XPL, _("Can't set SO_BROADCAST on socket %s (%d)"), strerror(errno), errno);
    close(sockfd);
    return EINA_FALSE;
  }
  ...
  {
    debug(MSG_XPL, _("Can't bind listener socket to port %d, %s (%d)"), xPLPort, strerror(errno), errno);
    close(sockfd);
    return EINA_FALSE;
  }
...
static Eina_Bool attempHubConnection()
{
  int sockfd;
  int flag = 1;
  ...
  {
    debug(MSG_XPL, _("Can't fetch socket info for bound listener, %s (%d)"), strerror(errno), errno);
    close(sockfd);
    return EINA_FALSE;
  }
...
static Eina_Bool makeConnection(xPL_ConnectType theConnectType) {
  ...
  if ((theConnectType == xcStandAlone) || (theConnectType == xcAuto)) {
    ...
    debug(MSG_XPL, _("Standalong connect failed - %d %d"), theConnectType, xcStandAlone);
...
/* found, then return EINA_FALSE.  Otherwise, install the name as */
/* the xPL interface and return EINA_TRUE                         */
static Eina_Bool findDefaultInterface(int sockfd)
{
  char intBuff[1024];
  struct ifconf ifc;
  struct ifreq *ifr;
...
setupBroadcastAddr()
{
  ...
  /* Map protocol name */
  if ((ppe = getprotobyname("udp")) == 0) {
    debug(MSG_XPL, _("Can't lookup UDP protocol info"));
    return EINA_FALSE;
  }
  ...
  {
    debug(MSG_XPL, _("Can't to create broadcast socket %s (%d)"), strerror(errno), errno);
    return EINA_FALSE;
  }
  ...
  if ((xPLInterfaceName == NULL) || (strlen(xPLInterfaceName) == 0))
  {
    ...
    {
      debug(MSG_XPL, _("Can't find a working, non-loopback network interface"));
      close(sockfd);
      return EINA_FALSE;
    }
  }
  ...
  {
    debug(MSG_XPL, _("Can't get IP addr for interface %s"), xPLInterfaceName);
    close(sockfd);
    return EINA_FALSE;
  }
  ...
  {
    debug(MSG_XPL, _("Can't extract the interface net mask"));
    close(sockfd);
    return EINA_FALSE;
  }
...
/* Send the passed string and return EINA_TRUE if it appears it */
/* or EINA_FALSE if there was an error                          */
Eina_Bool
xPL_sendRawMessage(String theData, int dataLen) {
  int bytesSent;
  ...
  if ((bytesSent = sendto(xPLBroadcastFD, theData, dataLen, 0,
			  ...
			  {
    debug(MSG_XPL, _("Can't broadcast message, %s (%d)"), strerror(errno), errno);
    return EINA_FALSE;
  }
...
xPL_initialize(xPL_ConnectType theConnectType)
{
  ...
  if (xPLFD != -1) return EINA_FALSE;

  /* Setup the broadcasting address */
  if (!setupBroadcastAddr()) return EINA_FALSE;

  /* Attempt to make our connection */
  if (!makeConnection(theConnectType)) return EINA_FALSE;
  ...
  if (!xPL_IODeviceInstalled) {
    if (xPL_addIODevice(xPL_receiveMessage, -1, xPLFD, EINA_TRUE, EINA_FALSE, EINA_FALSE))
      xPL_IODeviceInstalled = EINA_TRUE;
  }
...
Eina_Bool xPL_shutdown() {
  ...
  /* Remove xPL Listener */
  if (xPL_removeIODevice(xPLFD)) xPL_IODeviceInstalled = EINA_FALSE;
...
/* this call, EINA_TRUE is returned.  otherwise, EINA_FALSE.            */
Eina_Bool
xPL_processMessages(int theTimeout)
{
  Eina_Bool xPLMessageProcessed = EINA_FALSE;
  Eina_Bool timeoutDone = EINA_FALSE;
  ...
  for(;xPLFD != -1;) {
    ...
    if (activeDevices == -1)
    {
      ...
      debug(MSG_XPL, _("Can't poll devices - %s (%d) - terminating"), strerror(errno), errno);
