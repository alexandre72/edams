...
Eina_Bool xPL_removeRawListener(xPL_rawListener theHandler) {
  int listenerIndex;
  rawListenerPtr theListener;
...
Eina_Bool xPL_dispatchRawEvent(String theData, int dataLen) {
  Eina_Bool messageDispatched = EINA_FALSE;
...
Eina_Bool xPL_removeMessageListener(xPL_messageListener theHandler) {
  int listenerIndex;
  messageListenerPtr theListener;
...
/* Dispatch messages */
Eina_Bool xPL_dispatchMessageEvent(xPL_MessagePtr theMessage) {
  Eina_Bool messageDispatched = EINA_FALSE;
...
Eina_Bool xPL_removeServiceListener(xPL_ServicePtr theService, xPL_ServiceListener theListener) {
  int listenerIndex;
  xPL_ServiceListenerDefPtr theServiceListener;
...
Eina_Bool xPL_dispatchServiceEvent(xPL_ServicePtr theService, xPL_MessagePtr theMessage) {
  Eina_Bool messageDispatched = EINA_FALSE;
  ...
  for (listenerIndex = theService->listenerCount - 1; listenerIndex >= 0; listenerIndex--) {
    ...
    if (theListener->matchSchemaClass != NULL) {
      if (strcasecmp(theListener->matchSchemaClass, theMessage->schemaClass) != 0) continue;
    }

    /* Do a Schema type check */
    if (theListener->matchSchemaType != NULL) {
      if (strcasecmp(theListener->matchSchemaType, theMessage->schemaType) != 0) continue;
    }
...
Eina_Bool xPL_removeServiceConfigChangedListener(xPL_ServicePtr theService, xPL_ServiceConfigChangedListener theListener) {
  int listenerIndex;
  xPL_ServiceChangedListenerDefPtr theChangeListener;
...
/* Dispatch service messages to appropriate listeners*/
Eina_Bool xPL_dispatchServiceConfigChangedEvent(xPL_ServicePtr theService) {
  Eina_Bool messageDispatched = EINA_FALSE;
