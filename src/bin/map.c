    ...
    if(evas_object_focus_get(o) == EINA_TRUE)
    {
 		...
 		if(button_mask && (1 << 0))
 		{
			...
			if(evas_object_smart_parent_get(o))
			{
				...
				printf("Mouse pointer is in?%s\n", eina_rectangle_coords_inside(&prect, x, y)? "TRUE":"FALSE");
...
_evas_smart_example_smart_show(Evas_Object *o)
{
   EVAS_SMART_EXAMPLE_DATA_GET(o, priv);
	...
	for(x = 0;x != MAX_CHILD; x++)
	{
   		if (priv->children[x])	evas_object_show(priv->children[x]);
     }
...
_evas_smart_example_smart_hide(Evas_Object *o)
{
   EVAS_SMART_EXAMPLE_DATA_GET(o, priv);
	...
	for(x = 0;x != MAX_CHILD; x++)
	{
   		if (priv->children[x]) evas_object_hide(priv->children[x]);
    }
...
_evas_smart_example_smart_calculate(Evas_Object *o)
{
	...
	for(n = 0;n != MAX_CHILD; n++)
	{
   		if (priv->children[n])
     	{
        	...
        	evas_object_resize(priv->children[n], (w *0.20), (h *0.20));
